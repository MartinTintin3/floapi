...
export function fetchWithProgressTyped<O extends FloObject, R extends Relationships.Generic | Nothing, I = Exclude<FloObject, O> | Nothing>(url: string, onProgress?: (progress: number) => void): Promise<Responses.Base<O, R, I>> {
	return fetchWithProgress<Responses.Base<O, R, I>>(url, onProgress);
}

export function fetchWrestlersByAthleteId<R extends Relationships.ToWrestler | Nothing, I extends Exclude<FloObject, WrestlerObject> | Nothing>(id: UUID, config: FetchConfig, include: readonly WrestlersIncludeString[] = ["bracketPlacements.weightClass", "division", "event", "weightClass", "team"], extra?: string): Promise<Responses.Wrestlers<R, I>> {
...
export function fetchWrestlersByWeightClass<R extends Relationships.ToWrestler | Nothing, I extends Exclude<FloObject, WrestlerObject> | Nothing>(id: UUID, config: FetchConfig, include: readonly string[] = [], extra?: string): Promise<Responses.Wrestlers<R, I>> {
